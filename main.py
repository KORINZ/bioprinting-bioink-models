...
print(f'{"":#^100}')
print(f' Bioink inside a cylindrical needle; analytical solver based on power law, version {version_number}\n')
print(' #UPDATE LOG#\n', f'-{update_content}')
print(f'{"":#^100}')
...
def power_n(my_negative_list, n_minus_1=0):  # raise power of each item inside a list
    if n_minus_1 == -1:
        list_positive = [j ** (n - 1) for j in -1 * my_negative_list]  # avoid raising power for negative numbers
        return list(map(lambda var: var * -1, list_positive))  # change the signs back again
    else:
        list_positive = [j ** n for j in -1 * my_negative_list]  # avoid raising power for negative numbers
        return list(map(lambda var: var * -1, list_positive))  # change the signs back again


gamma_dot_n = power_n(dVzdr)  # shear rate raised to the power of n
tau_rz = list(map(lambda var: var * -K, gamma_dot_n))  # obtain shear stress (absolute value)
...
print("Volumetric Flow Rate =", round(Q_average_mL, 2), "[mL/s]\n")

Q_ave_mass_mg = Q_average * rho * 1e3
print("Mass Flow Rate =", round(Q_ave_mass_mg, 2), "[mg/s]\n")
...
print("Needle Center Line Velocity =", round(Vz_max_mm, 2), "[mm/s]; Average Extrusion Velocity =",
      round(V_average * 1e3, 2), "[mm/s]\n")

tau_max_kPa = (R / 2) * (Pn / Ln) / 1e3
print("Wall Shear Stress =", round(tau_max_kPa, 4), "[kPa]\n")

print("Max Shear Rate =", round(max(-dVzdr), 6), "[1/s];", "Min Shear Rate =", round(min(-dVzdr), 6), "[1/s]\n")

print("Max Viscosity =", round(max(eta), 4), "[Pa*s];", "Min Viscosity =", round(min(eta), 4), "[Pa*s]\n")
...
def read_data():
    try:
        ...
        print('No Data File Found. Please include the data csv file inside the data directory.')
...
xx_shearStress = np.linspace(1e-6, R * 1e6, len(df_shearStress))  # convert unit to micrometer
...
if plot_graphs:
    ...
    ###########################
    # Analytical Solution #
    plt.plot(x_neg_R_pos_R, np.concatenate([Vz(x_meter)[::-1] * 1e3, Vz(x_meter)[1:] * 1e3]), 'r', lw=linewidth,
             ls='--')  # (e3)=millimeter/s, (e6)=micrometer/s
    # OpenFOAM Simulation #
    plt.plot(np.concatenate([-xx_U[::-1], xx_U[1:]]), np.concatenate([df_U[::-1] * 1e3, df_U[1:] * 1e3]), 'g',
             lw=linewidth, ls='-', alpha=0.7)  # (e3)=millimeter/s, (e6)=micrometer/s
    ###########################
    ...
    fig1.legend(['Analytical Solution', 'OpenFOAM Simulation'], loc='lower center', bbox_to_anchor=(0.5, 0.15),
                prop=legend_size)

    if save_graphs:
        plt.savefig('figs_save/velocity_profile.png', dpi=dpi_save, bbox_inches='tight', pad_inches=0)
    ...
    fig2.legend(['Analytical Solution', 'OpenFOAM Simulation'], loc='lower center', bbox_to_anchor=(0.5, 0.15),
                prop=legend_size)
    ax = plt.gca()
    ax.set_yscale("log")  # enable log scale on the y-axis (Shear Rate)

    if save_graphs:
        plt.savefig('figs_save/shear_rate.png', dpi=dpi_save, bbox_inches='tight', pad_inches=0)
    ...
    if save_graphs:
        plt.savefig('figs_save/shear_stress.png', dpi=dpi_save, bbox_inches='tight', pad_inches=0)
    ...
    # OpenFOAM Simulation #
    axes[1].plot(df_shearRate, df_nu, 'g', lw=linewidth, ls='-', label="OpenFOAM Simulation", alpha=0.6)
    ...
    if save_graphs:
        plt.savefig('figs_save/apparent_viscosity.png', dpi=dpi_save, bbox_inches='tight', pad_inches=0)
    ...
    cpp = plt.contourf(x_neg_R_pos_R, x_neg_R_pos_R, Z_cat, levels=np.linspace(0, max(tau_rz), 75), cmap=cm.coolwarm,
    ...
    # plt.tight_layout()
    if plot_title:
        cp.suptitle(f"Contour Plot of {fig3a_name} (Analytical Solution) within Variable Radius, $-r$ to $r$",
                    weight=title_font_weight, size=11)
    for c in cpp.collections:
        c.set_edgecolor("face")

    if save_graphs:
        plt.savefig('figs_save/full_cut_contour.png', dpi=dpi_save, bbox_inches='tight', pad_inches=0)
